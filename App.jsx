...
function Dashboard({ user }) {
  ...
  // per-tenant loaded data
  const [tData,         setTData]         = useState({})  // { [tenantId]: { damages, serviceRequests, rentPayments, hoaCharges, checklistItems, depositReturn } }
  ...
  // ── Load detail for a tenant (all 5 sections)
  ...
  const createTenant = async () => {
    ...
    const row = {
      user_id: user.id,
      name: tF.name.trim(), unit: tF.unit.trim(),
    ...
    // Insert default checklist via DB function
  ...
  const deleteTenant = async (id) => {
    if (!window.confirm('Delete this tenant and all records permanently?')) return
    await supabase.from('tenants').delete().eq('id', id)
    setTenants(prev => prev.filter(t => t.id !== id))
    if (selId === id) { setSelId(null); setPanel('splash') }
  }

  // ── Generic helpers to add/remove records in each table
  ...
  // ── CHECKLIST
  const updCLItem = async (itemId, patch) => {
    await updRecord('checklist_items', itemId, patch, 'checklistItems')
  }
  const addCLItem = async () => {
    ...
    await addRecord('checklist_items', {
      area: newCI.area.trim(), item: newCI.item.trim(),
      condition: 'good', count_ded: false, cost: 0, sort_order: maxOrder+1,
    }, 'checklistItems')
    setNewCI({area:'',item:''})
  }
  const delCLItem = async (id) => {
    await delRecord('checklist_items', id, 'checklistItems')
  }
  ...
  const doExport = () => {
    ...
    exportTenantReport(T, {
      damages:         TD.damages,
      serviceRequests: TD.serviceRequests,
      rentPayments:    TD.rentPayments,
      hoaCharges:      TD.hoaCharges,
      checklistItems:  TD.checklistItems,
      depositReturn:   TD.depositReturn,
    })
  }
    ...
    <div style={{fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',fontSize:14,minHeight:'100vh',background:'#f8fafc'}}>
      ...
      <div style={{background:'#0f1e3c',padding:'13px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:50}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          {T && <button onClick={doExport} style={{background:'#16a34a',color:'#fff',border:'none',borderRadius:6,padding:'6px 13px',fontSize:12,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',gap:4}}>
            <i className="ti ti-file-spreadsheet" style={{fontSize:13}}/> Export All
          </button>}
          <button onClick={()=>setPanel('new-tenant')} style={{background:'#f59e0b',color:'#fff',border:'none',borderRadius:6,padding:'6px 14px',fontSize:13,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',gap:5}}>
            <i className="ti ti-plus" style={{fontSize:13}}/> New Tenant
          </button>
          <button onClick={()=>supabase.auth.signOut()} title="Sign out" style={{background:'transparent',border:'1px solid #334155',color:'#94a3b8',borderRadius:6,padding:'6px 10px',fontSize:12,cursor:'pointer'}}>
          ...
          {panel==='detail' && T && <div style={{background:'#fff',minHeight:'calc(100vh - 50px)'}}>
            ...
            {!dataLoading && <div style={{padding:'16px 20px'}}>
              ...
              {tab==='damages' && (() => {
                ...
                return <div>
                  ...
                  {T.status==='active' && <div style={{...CS,background:'#f8fafc',marginBottom:16}}>
                    ...
                    <button onClick={addDamage} disabled={!dF.description.trim()||imgUploading} style={{marginTop:12,background:'#0f1e3c',color:'#fff',border:'none',borderRadius:6,padding:'7px 16px',fontSize:13,fontWeight:500,cursor:'pointer',opacity:dF.description.trim()&&!imgUploading?1:0.5}}>
                      {imgUploading?'Uploading…':'+ Add Damage Item'}
                    </button>
                  </div>}
              ...
              {tab==='service' && (() => {
                ...
                return <div>
                  <div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap'}}>
                    {mc('Total Requests',svcs.length)}
                    {mc('Open',svcs.filter(s=>s.status==='Open').length,'#d97706')}
                    {mc('Total Cost',fmt(tot),'#dc2626')}
                    {mc('Completed',svcs.filter(s=>s.status==='Completed').length,'#16a34a')}
                  </div>
                  {T.status==='active' && <div style={{...CS,background:'#f8fafc',marginBottom:16}}>
                    ...
                    <button onClick={addService} disabled={!sF.description.trim()||imgUploading} style={{marginTop:12,background:'#0f1e3c',color:'#fff',border:'none',borderRadius:6,padding:'7px 16px',fontSize:13,fontWeight:500,cursor:'pointer',opacity:sF.description.trim()&&!imgUploading?1:0.5}}>
                      {imgUploading?'Uploading…':'+ Log Request'}
                    </button>
                  </div>}
              ...
              {tab==='rent' && (() => {
                ...
                return <div>
                  <div style={{display:'flex',gap:10,marginBottom:4,flexWrap:'wrap'}}>
                    {mc('Total Collected',fmt(totCol),'#16a34a')}
                    {mc('Late Fees',fmt(totLF),'#d97706')}
                    {mc('Admin Fees',fmt(totAF),'#0284c7')}
                    {mc('Payments',rents.length)}
                  </div>
                  ...
                  {T.status==='active' && <div style={{...CS,background:'#f8fafc',marginBottom:16}}>
                    ...
                    {formIsLate && <div style={{marginBottom:12,padding:'9px 12px',background:'#fef3c7',border:'1px solid #f59e0b',borderRadius:6,display:'flex',alignItems:'flex-start',gap:8}}>
                      ...
                      <div><div style={{fontWeight:500,fontSize:13,color:'#92400e'}}>Late — {formDaysLate} day{formDaysLate!==1?'s':''} past 3-day grace</div><div style={{fontSize:12,color:'#78350f',marginTop:2}}>Late fee and admin fee pre-filled from tenant defaults. Adjust as needed.</div></div>
                    </div>}
              ...
              {tab==='hoa' && (() => {
                ...
                return <div>
                  <div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap'}}>
                    {mc('Outstanding',fmt(totOut),'#dc2626')}
                    {mc('Paid',fmt(totPd),'#16a34a')}
                    {mc('Total Charges',hoas.length)}
                  </div>
                  {T.status==='active' && <div style={{...CS,background:'#f8fafc',marginBottom:16}}>
              ...
              {tab==='deposit' && (() => {
                ...
                return <div>
                  <div style={{display:'flex',gap:10,marginBottom:12,flexWrap:'wrap'}}>
                    {mc('Deposit Held',fmt(deposit),'#64748b')}
                    {mc('Total Deductions',fmt(dedTotal),'#dc2626')}
                    {mc('Return to Tenant',fmt(returnAmt),'#16a34a')}
                    {mc('Damaged Items',dedItems.length,'#d97706')}
                  </div>
                  <div style={{fontSize:12,color:'#64748b',marginBottom:14,padding:'8px 12px',background:'#f0f9ff',borderLeft:'3px solid #0284c7',borderRadius:'0 6px 6px 0'}}>
                    <strong>Note:</strong> Normal wear &amp; tear is <em>not</em> deductible. Mark "Damaged" + check "Deduct from deposit" for actual damage — then enter cost, company, and invoice #.
                  </div>
